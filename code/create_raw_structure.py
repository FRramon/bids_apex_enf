...
import glob 
import datetime
### L'idee est de fusionner les deux csv (dicom et recp/par)
## Créer une structure avec les noms sub-enfxxxx-apexid
## Cela va permettre de voir si il y a des dossiers avec le meme id sub-enf, et un apexid différent, ou inversement.
## A l'intérieur je crée un csv avec toute les sequences correspondantes : tri par la suite des sessions avec les dates.
...
df_recpar = pd.read_csv("/Volumes/My Passport/francois/scan_folders_recpar.csv")
df_dicom = pd.read_csv("/Volumes/My Passport/francois/scan_folders_dicom.csv")
df  = pd.concat([df_recpar,df_dicom])
df["concat_apex_id"] = df["subject_id"] + "_" + df["patient_name"]

### Create structure  ###

raw_structure_dir = "/Volumes/My Passport/raw_structure"
...
if not os.path.isdir(raw_structure_dir):

	for ids in tqdm(unique_subject_ids):
		os.makedirs(os.path.join(raw_structure_dir,ids),exist_ok = True)

		df_subject_id = filter_by_subject_id(df,ids)
...
## Check ids associated: one or more associated with a apex id  ###
...
if correctNames: 
	...
	df_correct_names.to_csv("/Volumes/My Passport/francois/names_corrected.csv")
...
if rename_session:
	for ids in unique_corrected_names:
...
if createSummary:
	...
	df_summary.to_csv("/Volumes/My Passport/francois/summary_mri.csv")
